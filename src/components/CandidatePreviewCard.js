...
const CandidatePreviewCard = ({ children, candidate, source = 'pipeline', pipelineData = null }) => {
    ...
    const handleMouseEnter = () => {
        ...
        hoverTimer.current = setTimeout(() => {
            setShowPreview(true);
            // calculatePosition() is now handled by useLayoutEffect when showPreview becomes true
    ...
    React.useLayoutEffect(() => {
        if (showPreview && previewRef.current && triggerRef.current) {
            ...
            // Vertical Logic:
            // 1. Default: Below the trigger
            // 2. If runs off bottom, try above trigger
            if (top + previewRect.height > viewportHeight) {
                const topSpace = triggerRect.top;
                const bottomSpace = viewportHeight - triggerRect.bottom;

                // If more space above, put it above
                if (topSpace > bottomSpace) {
                    top = triggerRect.top - previewRect.height - padding;
                } else {
                    // Otherwise keep below but clamp to bottom edge (might cover trigger, but visible)
                    top = viewportHeight - previewRect.height - padding;
                }
            }

            // Final vertical clamp to ensure it never goes off top
            if (top < padding) top = padding;


            // Horizontal Logic:
            // 1. Default: Align left with trigger
            // 2. If runs off right, align right with viewport
            if (left + previewRect.width > viewportWidth) {
                left = viewportWidth - previewRect.width - padding;
            }

            // Final horizontal clamp to ensure it never goes off left
