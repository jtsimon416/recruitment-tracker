...
function TalentPool() {
      ...
      if (candidateFormData.resume_url && candidateFormData.resume_url.endsWith('.docx')) {
        try {
          const { data: signedUrlData, error: signedUrlError } = await supabase.storage
            .from(BUCKET_NAME)
            .createSignedUrl(candidateFormData.resume_url, 60);
  ...
  useEffect(() => {
    if (!loadingSession && user) { // Only load data if session is not loading and user is authenticated
      loadData();
    }
  }, [loadData, user, loadingSession]); // Add user and loadingSession to dependencies
  ...
  async function handleFileUpload(e) {
    ...
    try {
      ...
      if (error) throw error;
      setCandidateFormData(prev => ({ ...prev, resume_url: filePath }));
      e.target.value = null;
  ...
  async function handleManualSubmit(e) {
    ...
    if (dataToSave.email) {
      const { data: existing, error: checkError } = await supabase.from('candidates').select('*').eq('email', dataToSave.email).single();
      ...
      const { data: existing, error: checkError } = await supabase.from('candidates').select('*').eq('linkedin_url', dataToSave.linkedin_url).single();
      if (checkError && checkError.code !== 'PGRST116') {
        showConfirmation({ type: 'error', title: 'Error', message: `Could not check for duplicates: ${checkError.message}` });
        setLoading(false);
        return;
  ...
  const openPipelineModal = (candidate) => {
    ...
    setPipelineData({ position_id: '', stage: 'Screening', candidate_id: candidate.id, is_video_screened: null, video_screen_reason: '' });
  ...
  const handlePipelineSubmit = async (e) => {
    ...
    if (pipelineData.is_video_screened === null) {
      showConfirmation({ type: 'error', title: 'Error', message: 'Please indicate if the candidate was video screened.' });
      return;
    }
    if (pipelineData.is_video_screened === false && !pipelineData.video_screen_reason.trim()) {
      showConfirmation({ type: 'error', title: 'Error', message: 'Please provide a reason why the video screen was not done.' });
      return;
    }
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
  ...
  const handleResumeClick = async (e, candidate) => { // Make it async
    e.preventDefault();
    e.stopPropagation();

    if (!candidate.resume_url) return; // No resume path, nothing to do

    setLoading(true); // Show loading state while generating signed URL
    try {
      const { data: signedUrlData, error: signedUrlError } = await supabase.storage
        .from(BUCKET_NAME)
        .createSignedUrl(candidate.resume_url, 60); // 60 seconds expiry
      ...
      if (candidate.resume_url.endsWith('.docx')) {
        setWordDocUrl(signedUrl); // Pass signed URL to modal
        setWordDocCandidateName(candidate.name);
        setShowWordDocModal(true);
      } else if (candidate.resume_url.endsWith('.pdf')) { // Explicitly check for PDF
        window.open(signedUrl, '_blank'); // Open signed URL for PDF
      } else {
        // Fallback for other types or if extension is missing
        window.open(signedUrl, '_blank');
      }
    } catch (error) {
      console.error('Error generating signed URL:', error);
      showConfirmation({ type: 'error', title: 'Error', message: `Could not view resume: ${error.message}` });
    } finally {
      setLoading(false);
    }
  };
  ...
  async function handleAiParseSubmit(file) {
    ...
    try {
      const [resumeText, publicUrl] = await Promise.all([
      ...
      const skillsArray = typeof structuredData.skills === 'string'
        ? structuredData.skills.split(',').map(skill => skill.trim()).filter(Boolean)
      ...
      setCandidateFormData({
        ...
        resume_url: publicUrl, // Use the URL from the upload
        document_type: 'Resume',
        created_by_recruiter: ''
      });
