...
function DirectorReview() {
    const { user, userProfile, createNotification, pipeline, refreshData, loading: dataLoading } = useData();
    ...
    const candidatesForReview = useMemo(() => {
        ...
        // Sort the pipeline entries themselves by creation date, descending (newest first)
    ...
    const handleUpdateComment = async () => {
        ...
        if (error) {
            ...
            await refreshData(); // Fetch to get correct state on error
        } else {
    ...
    const handleFinalDecision = async (pipelineEntry, action, commentText) => {
        ...
        await refreshData(); // Re-fetch to update lists
        // setLoading(false);
        closeCommentsModal();
    };
    ...
    const handleAnalyzeFit = async (pipelineItem) => {
        ...
        try {
            ...
            const { data: candidateData, error: candidateError } = await supabase
                ...
                .eq('id', pipelineItem.candidates.id)
                .single();

            if (candidateError) throw candidateError;
            if (!candidateData?.resume_url) throw new Error('Candidate has no resume uploaded.');

            // 2. Fetch Position Data (for description)
            const { data: positionData, error: positionError } = await supabase
                .from('positions')
                .select('description')
                .eq('id', pipelineItem.positions.id)
                .single();
            ...
            const response = await supabase.functions.invoke('hire-logic-ai', {
                body: JSON.stringify({
                    resumeText: resumeText,
                    jobDescriptionText: positionData.description,
                }),
            });
    ...
    if (dataLoading) return <div className="loading-state">Loading reviews...</div>;
                    ...
                    return (
                        <div key={p.id} className={`candidate-card ${isAging ? 'card-aging' : ''}`}>
                            ...
                            <div className="card-actions">
                                ...
                                <button className="btn btn-analyze-fit" onClick={() => handleAnalyzeFit(p)} title="Hire Logic AI Analysis">
                                    <Sparkles size={16} />
                                </button>
                            </div>
                        </div>
