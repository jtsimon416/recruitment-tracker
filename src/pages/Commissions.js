...
function Commissions() {
  ...
  const { updateCommissionStatus, pipeline } = useData();
  ...
  const addFormRef = useRef(null); // Ref for the form card
  ...
  // When a position is selected in the form, filter the candidate dropdown
  useEffect(() => {
    if (formData.position_id && pipeline.length > 0) {
      const candidatesInPosition = pipeline
        .filter(p => p.position_id === formData.position_id)
        .map(p => p.candidates);

      // Filter out null/undefined candidates and get unique candidates
      const uniqueCandidates = Array.from(new Map(candidatesInPosition.filter(c => c).map(c => [c.id, c])).values());

      setFilteredCandidates(uniqueCandidates);
    } else {
      setFilteredCandidates([]); // Clear if no position is selected
    }
    // Reset candidate selection when position changes
    setFormData(prevData => ({ ...prevData, candidate_id: '' }));
  }, [formData.position_id, pipeline]);
    ...
    <div className="page-container">
      ...
      <div className="filter-bar">
        ...
        <div className="form-group filter-group">
            ...
            <button className="btn-secondary btn-clear-filters" onClick={handleClearFilters}>
              Clear Filters
            </button>
        </div>
      </div>
      ...
      {showForm && (
        <div className="form-card" ref={addFormRef}>
