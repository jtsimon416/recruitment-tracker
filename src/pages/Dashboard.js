...
// Custom Tooltip Component for Role Health Distribution
const CustomRoleHealthTooltip = ({ active, payload, label }) => {
  console.log('--- DEBUG: CustomRoleHealthTooltip - active:', active, 'payload:', payload); // New log
  if (active && payload && payload.length) {
    const data = payload[0].payload; // The original data object for the hovered segment
    const name = data.name;
    const value = data.value;
    const roles = data.roles || []; // Get the list of roles
    console.log('--- DEBUG: CustomRoleHealthTooltip - data.roles:', data.roles, 'extracted roles:', roles); // New log

    let description = '';
    let action = '';
    switch (name) {
      case 'Healthy':
        description = 'Roles with good recent activity and reply rates.';
        action = 'Keep up the great work!';
        break;
      case 'Warning':
        description = 'Roles with some recent activity but potentially low engagement or older activity.';
        action = 'Consider increasing outreach or checking candidate quality.';
        break;
      case 'Critical':
        description = 'Roles with very low or no recent activity, or very poor reply rates.';
        action = 'Immediate attention required! Review strategy and candidate pipeline.';
        break;
      default:
        description = '';
        action = '';
    ...
    return (
      <div style={{
        ...
        <p style={{ fontWeight: 'bold', marginBottom: '5px' }}>{`${name}: ${value} roles`}</p>
        {roles.length > 0 && (
          <div style={{ marginTop: '10px', borderTop: '1px solid #333', paddingTop: '10px' }}>
            <p style={{ fontWeight: 'bold', marginBottom: '5px' }}>Roles:</p>
            <ul style={{ listStyleType: 'disc', marginLeft: '20px', padding: 0 }}>
              {roles.slice(0, 5).map((roleTitle, index) => (
                <li key={index}>{roleTitle}</li>
              ))}
              {roles.length > 5 && <li>{`and ${roles.length - 5} more...`}</li>}
            </ul>
          </div>
        )}
        <p style={{ marginTop: '10px', marginBottom: '5px' }}>{description}</p>
        <p style={{ fontStyle: 'italic' }}><strong>Action:</strong> {action}</p>
      </div>
    );
  }
  return null;
...
// PERFORMANCE TAB
// =========================================================================
const PerformanceTab = ({ stats, historicalData, roleHealth, groupedRolesByHealth }) => {
  console.log('--- DEBUG: PerformanceTab - received groupedRolesByHealth prop:', groupedRolesByHealth); // New log
  ...
  const healthDistribution = useMemo(() => {
    ...
    const healthDistData = [
      { name: 'Healthy', value: distribution.healthy, color: '#B8D4D0', roles: groupedRolesByHealth.Healthy },
      { name: 'Warning', value: distribution.warning, color: '#F4C2A8', roles: groupedRolesByHealth.Warning },
      { name: 'Critical', value: distribution.critical, color: '#F7A9BA', roles: groupedRolesByHealth.Critical }
    ];
    console.log('--- DEBUG: PerformanceTab - healthDistribution data:', healthDistData); // New log
    return healthDistData;
  }, [roleHealth, groupedRolesByHealth]); // Add groupedRolesByHealth to dependencies
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              ...
              <Tooltip
                content={<CustomRoleHealthTooltip />}
              />
            </PieChart>
          </ResponsiveContainer>
        </motion.div>
...
function Dashboard() {
  ...
  const [groupedRolesByHealth, setGroupedRolesByHealth] = useState({ Healthy: [], Warning: [], Critical: [] }); // New state
  const [weeklyOutreachByPosition, setWeeklyOutreachByPosition] = useState([]);
  ...
  }, [userProfile, positions, pipeline, recruiters, outreachActivities]);

  useEffect(() => {
    if (!dataContextLoading) {
      fetchAllData();
    }
  }, [dataContextLoading, fetchAllData]);
  ...
  async function calculateRoleHealth(metrics, pipelineData) {
    ...
    // Group roles by health status for the tooltip
    const groupedRoles = {
      Healthy: [],
      Warning: [],
      Critical: [],
    };
    Object.values(healthMap).forEach(role => {
      if (groupedRoles[role.health]) {
        groupedRoles[role.health].push(role.title);
      }
    });
    console.log('--- DEBUG: calculateRoleHealth - groupedRoles before setting state:', groupedRoles); // New log
    setGroupedRolesByHealth(groupedRoles); // Set the new state
  }

  async function fetchExecutiveStats() {
    // Correctly define the current week (Sunday to Saturday)
    const now = new Date();
  ...
  async function fetchWeeklyOutreachByPosition() {
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const dayOfWeek = today.getDay(); // 0=Sunday
    const startOfWeek = new Date(today);
    ...
    if (error || !outreachData) {
      ...
      setWeeklyOutreachByPosition([]);
      return;
    }

    const outreachByPosition = outreachData.reduce((acc, outreach) => {
      if (outreach.positions) {
        const positionTitle = outreach.positions.title;
        acc[positionTitle] = (acc[positionTitle] || 0) + 1;
      }
      return acc;
    }, {});

    const formattedData = Object.entries(outreachByPosition)
      .map(([title, count]) => ({ title, count }))
      .sort((a, b) => b.count - a.count);

    setWeeklyOutreachByPosition(formattedData);
  }
  ...
  const getSubmissionsThisWeekTooltip = useMemo(() => {
    ...
    const recentSubmissions = pipeline
      .filter(p => p.stage === 'Submit to Client' && new Date(p.created_at) >= startOfWeek)
    ...
    if (pipeline.filter(p => p.stage === 'Submit to Client' && new Date(p.created_at) >= startOfWeek).length > 6) {
      items.push({
        primary: `and ${pipeline.filter(p => p.stage === 'Submit to Client' && new Date(p.created_at) >= startOfWeek).length - 6} more...`,
        secondary: null
      });
    }
      ...
      {activeTab === 'performance' && (
        <div>
          <PerformanceTab
            ...
            groupedRolesByHealth={groupedRolesByHealth} // Pass new prop
          />
        </div>
      )}
